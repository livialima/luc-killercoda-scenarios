...
But if you want more info on that traffic, `sudo iftop -i enp1s0`{{exec}} is a [nice display](https://manpages.ubuntu.com/manpages/xenial/man8/iftop.8.html). *Change `enp1s0` for the interface you wish to capture traffic information.* To exit the monitor view, type `q` to quit.